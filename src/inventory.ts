import { InventoryItem, ItemId, ITEM_METADATA } from 'shared';
 ...
 */
export function addToInventory(inventory: InventoryItem[], itemId: ItemId, count: number): boolean {
  const meta = ITEM_METADATA[itemId];
  if (!meta || itemId === ItemId.NONE) return false;
 ...
 */
export function hasInInventory(inventory: InventoryItem[], itemId: ItemId, count: number): boolean {
 ...
 */
export function removeFromInventory(inventory: InventoryItem[], itemId: ItemId, count: number): boolean {
  if (!hasInInventory(inventory, itemId, count)) return false;
