...
function findSpawnPosition(worldMap) {
    const centerCol = Math.floor(shared_1.WORLD_WIDTH / 2);
    const centerRow = Math.floor(shared_1.WORLD_HEIGHT / 2);
    ...
    for (let r = 0; r < 20; r++) {
        for (let dx = -r; dx <= r; dx++) {
            for (let dy = -r; dy <= r; dy++) {
                ...
                if (tx > 0 && tx < shared_1.WORLD_WIDTH - 1 && ty > 0 && ty < shared_1.WORLD_HEIGHT - 1) {
                    const idx = ty * shared_1.WORLD_WIDTH + tx;
                    if (worldMap[idx] === shared_1.TileType.GRASS) {
                        return {
                            x: tx * shared_1.TILE_SIZE + shared_1.TILE_SIZE / 2,
                            y: ty * shared_1.TILE_SIZE + shared_1.TILE_SIZE / 2,
                        };
                    }
                }
            }
    ...
    // Fallback
    return { x: 50 * shared_1.TILE_SIZE + 16, y: 50 * shared_1.TILE_SIZE + 16 };
}
// Get or create session
function getOrCreateSession(instanceId) {
    if (!sessions[instanceId]) {
        console.log(`Generating new world for instance ${instanceId}...`);
        const worldMap = (0, world_1.generateWorld)(instanceId);
        sessions[instanceId] = {
            instanceId,
            worldMap,
            players: {},
...
wss.on('connection', (ws, request) => {
    ...
    if (!player) {
        ...
        player = {
            ...
            health: 100,
            hunger: 100,
            activeItem: shared_1.ItemId.NONE,
            inventory: Array.from({ length: 20 }, () => ({ itemId: shared_1.ItemId.NONE, count: 0 })),
        };
        // Give some starting items for testing
        player.inventory[0] = { itemId: shared_1.ItemId.APPLE, count: 5 };
        player.inventory[1] = { itemId: shared_1.ItemId.WOOD, count: 10 };
        player.inventory[2] = { itemId: shared_1.ItemId.STONE, count: 5 };
    }
    session.players[userId] = player;
    session.clients.set(userId, ws);
    // Send INIT message to client
    const initMsg = {
        type: shared_1.ServerMessageType.INIT,
        playerId: userId,
        worldWidth: shared_1.WORLD_WIDTH,
        worldHeight: shared_1.WORLD_HEIGHT,
    ...
    ws.on('close', () => {
        ...
        const leaveMsg = {
            type: shared_1.ServerMessageType.PLAYER_LEFT,
            playerId: userId,
        };
...
function handleClientMessage(session, userId, msg) {
    const player = session.players[userId];
    if (!player)
        return;
    switch (msg.type) {
        case shared_1.ClientMessageType.MOVE: {
            const newX = msg.x;
            const newY = msg.y;
            ...
            if (dist < 40) {
                if (!(0, physics_1.checkCollision)(newX, newY, session.worldMap)) {
                    player.x = newX;
                    player.y = newY;
                }
            }
        ...
        case shared_1.ClientMessageType.MINE: {
            const tx = msg.x;
            const ty = msg.y;
            // Validate range: Player center to tile center distance
            const tileCenterX = tx * shared_1.TILE_SIZE + shared_1.TILE_SIZE / 2;
            const tileCenterY = ty * shared_1.TILE_SIZE + shared_1.TILE_SIZE / 2;
            ...
            let damage = 1;
            const activeItemMeta = shared_1.ITEM_METADATA[player.activeItem];
            ...
            if (health <= 0) {
                ...
                let replacement = shared_1.TileType.EMPTY;
                if (tileType === shared_1.TileType.TREE) {
                    replacement = shared_1.TileType.GRASS;
                }
                else if (tileType === shared_1.TileType.ROCK) {
                    replacement = shared_1.TileType.GRASS;
                }
                else if (tileType === shared_1.TileType.IRON_ORE_NODE) {
                    replacement = shared_1.TileType.STONE;
                }
                session.worldMap[idx] = replacement;
                session.dirtyTiles.push({ x: tx, y: ty, type: replacement });
                // Award resource
                if (meta.yieldItem !== shared_1.ItemId.NONE && meta.yieldCount > 0) {
                    const added = (0, inventory_1.addToInventory)(player.inventory, meta.yieldItem, meta.yieldCount);
                    if (added) {
                        // Send inventory update to this client only
                        const ws = session.clients.get(userId);
                        if (ws && ws.readyState === ws_1.WebSocket.OPEN) {
                            const invMsg = {
                                type: shared_1.ServerMessageType.INVENTORY_UPDATE,
        ...
        case shared_1.ClientMessageType.PLACE: {
            const tx = msg.x;
            const ty = msg.y;
            ...
            if (currentTile !== shared_1.TileType.EMPTY)
                break;
            // Verify player inventory owns this item
            if (!(0, inventory_1.hasInInventory)(player.inventory, itemId, 1))
                break;
            const itemMeta = shared_1.ITEM_METADATA[itemId];
            if (!itemMeta || !itemMeta.isPlaceable || itemMeta.placesTile === undefined)
                break;
            // Check if any player resides on the target tile to prevent trapping/suffocation
            let isOccupied = false;
            const tileLeft = tx * shared_1.TILE_SIZE;
            const tileRight = (tx + 1) * shared_1.TILE_SIZE;
            const tileTop = ty * shared_1.TILE_SIZE;
            const tileBottom = (ty + 1) * shared_1.TILE_SIZE;
            ...
            if (isOccupied)
                break;
            // Place block!
            const success = (0, inventory_1.removeFromInventory)(player.inventory, itemId, 1);
            if (success) {
        ...
        case shared_1.ClientMessageType.SELECT_ITEM: {
            const itemId = msg.itemId;
            if (itemId === shared_1.ItemId.NONE || (0, inventory_1.hasInInventory)(player.inventory, itemId, 1)) {
...
setInterval(() => {
    for (const instanceId in sessions) {
        ...
        const now = Date.now();
        const timeOfDay = (now % shared_1.DAY_DURATION_MS) / shared_1.DAY_DURATION_MS;
        // Send tick updates
        const tickMsg = {
            type: shared_1.ServerMessageType.TICK,
            timeOfDay,
        ...
        if (session.dirtyTiles.length > 0) {
            const tileUpdateMsg = {
                type: shared_1.ServerMessageType.TILE_UPDATE,
